...
def parse_verdict(raw_text: str, *, request: "AnalysisRequest", role_id: str) -> tuple[LLMVerdict | None, tuple[ContractViolation, ...]]:
...
class MockLLMClient:
    ...
    def capabilities(self) -> LLMCapabilities:
        return LLMCapabilities(
            provider="mock", deterministic=True, requires_credentials=False,
            max_input_chars=200_000, supports_json_mode=True,
        )

    def complete(self, prompt: Prompt, *, schema=LLMVerdict, timeout_s: float = 30.0) -> LLMResponse: ...
```
...
| 单文件 300 行 / 顶层依赖 ≤ 8 / py 文件 ≤ 80 | 工程约束 | **纪律** | CI 断言 |
...
| **R-E** | 阶段闸门与质量门的耦合 | 某 risk_essential 维度 BLOCKED，`gate_allows_post()` 返回 False | POST 阶段整体不执行，用户看到"什么都没跑" | `on_blocked` 三态（`abort` / `skip` / `degrade`）下放到步骤级；闸门拒绝时必须打印**是哪个维度、哪条断言**导致，且 `coverage` 命令可复现 | 否 |
...
| **R-J** | 文件规模 76 > `ALPHA_BOUNDARY` 承诺的"约 60" | v0.2 落地 15 个新 py | 文档自相矛盾，且"精细切割"的克制叙事被削弱 | 在 `ALPHA_BOUNDARY.md` 五章补一行 **v0.2 目标值 ≤ 80**，并注明增量全部为骨架/契约文件（无 alpha）；若主理人要求守住 60，需砍 P2-1 与部分 CLI 拆分 | **是**（开放问题 O-4） |
...
| **O-4** | 文件规模承诺 | A 把 `ALPHA_BOUNDARY` 改为 ≤80／B 守住 60，砍 P2-1 与 CLI 拆分 | **A**。60 是 v0.1 的实测值不是设计目标；把契约文件算进"膨胀"会诱导把多个契约塞进一个文件，反而伤可读性 | T0（文档口径）——建议开工前拍 |
