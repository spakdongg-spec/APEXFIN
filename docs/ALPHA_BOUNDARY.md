...
| `signal_arbiter.py`、`promote_signals.py`、`evidence_fusion.py`、`evidence_fusion_canary.py`、`signal_registry.py` | `decision/debate.py` + `core/registry.py` | 「多信号 -> 单决策」的裁决位置；PM 加权裁判（AFFIRM/MODIFY/REJECT） | 所有权重、优先级、仲裁规则。替换为置信×角色权重的公开加权，**刻意不提供可调参数**（有参数就有 alpha 嫌疑） |
